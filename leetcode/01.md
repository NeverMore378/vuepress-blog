...
```java
    ...
    void dfs1(TreeNode root, int cur, int sum) {
        if (root == null) {
            return;
        }
        // 每个节点都进行一次求和的dfs
        dfs(root, cur, sum);
        dfs1(root.left, cur, sum);
        dfs1(root.right, cur, sum);
    }
...
```java
    ...
    public void dfs(TreeNode root, LinkedList<TreeNode> queue, LinkedList<Integer> path){
        if (root == null) return;
...
```java
    ...
    public Node treeToDoublyList(Node root) {
        ...
        dfs(root);
        // 遍历结束后，pre来到了最后，将头节点左指针指向它，pre的右节点指向头节点
        head.left = pre;
        pre.right = head;
        return head;
    }
