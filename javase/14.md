...
首先，能(s1, s2) -> s1.compareToIgnoreCase(s2)这样写，是因为sorted里面要实现Comparator接口里面的int compare(T o1, T o2)方法

s -> System.out.println(s)是要实现Consumer接口里面的void accept(T t)方法

这两个接口都是函数式接口，所以能用lambda代替匿名内部类

其次，上述程序生成一个Stream流，对流中的字符串进行排序并遍历打印，程序中采用lambda表达式的方式代替匿名类简化了代码

然而代码中两处lambda表达式都仅仅调用一个已存在的方法：String.compareToIgnoreCase、System.out.println，这种情况可以用方法引用来简化
