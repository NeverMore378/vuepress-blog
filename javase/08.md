...
public class Test<T> {
    ...
    // 是泛型方法
    public <W> W demo2(W t) {
        return t;
    }

    // 静态方法使用泛型参数会报错
    // 静态方法是属于类的，类直接调用的话，T类型无法指定具体类型，那么该方法就没有意义，所以直接报错
	public static T demo3(T t) { return t;}

    // 定义泛型静态方法
    public static <W> void demo4(W w) {
        System.out.println(w);
    }
    
    // 两个静态方法的区别就是一个是使用泛型参数，一个是定义泛型方法
}
```
