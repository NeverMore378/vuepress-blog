...
private static final ThreadLocal<SimpleDateFormat> LOCAL = new ThreadLocal<SimpleDateFormat>() {
    ...
    // remove()后再去get拿到的threadLocalMap还是之前的threadLocalMap，但是从里面拿到的entry不是同一个了(entry里面的k->threadlocal对象始终是同一个，但是v->SimpleDateFormat不是同一个)
    // 如果不remove()去get的话拿到的entry是同一个(threadlocal对象始终是同一个,拿到的SimpleDateFormat也是同一个)
    LOCAL.remove();
```
