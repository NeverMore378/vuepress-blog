...
- 2.父类非静态块->父类成员变量->父类构造函数
  
- 3.子类非静态块->子类成员变量->子类构造函数


### 强引用、软引用、弱引用、虚引用

- 强引用：new、反射，JVM不会回收，宁愿OOM

- 软引用：内存足够的时候不会回收、内存不够回收掉

```java
    SoftReference<Byte[]> softReference = new SoftReference<>(new Byte[1024 * 1024]);
    // [Ljava.lang.Byte;@1f89ab83 能拿到
    System.out.println(softReference.get());
```

像这种如果内存充足，GC时就保留，内存不够，GC再来收集的功能很适合用在缓存的引用场景中。

在使用缓存时有一个原则，如果缓存中有就从缓存获取，如果没有就从数据库中获取，缓存的存在是为了加快计算速度，如果因为缓存导致了内存不足进而整个程序崩溃，那就得不偿失了

- 弱引用：无论内存是否充足，都会被回收

  - ThreadLocal内部是ThreadLocalMap，ThreadLocalMap的成员变量private Entry[] table，Entry继承了弱引用，Entry的k是一个弱引用，v是强引用
  - Entry(在堆里)的k指向ThreadLocal(在堆里)是一个虚线，如果是实线的话，ThreadLocalRef(在栈里)本来是指向ThreadLocal(在堆里)这条线断掉(= null)，那么ThreadLocal(在堆里)就不会被回收，因为被Entry(在堆里)的k强引用
  - 注意一下CurrentThreadRef(在栈里)指向CurrentThread(在堆里)，CurrentThread(在堆里)指向ThreadLocalMap(在堆里)，ThreadLocalMap里面的Entry里面的v是强引用，这条链不会断掉，要手动remove()一下
...
- 虚引用
  ...
  - 虚引用无论gc都拿不到它的的值，phantomReference.get()都是Null，用处：堆外内存回收
  - nio可以直接调用堆外内存，gc回收不了堆外内存
  - 虚引用要配和队列去使用
...
```java
    public static void main(String[] args) {
        ...
        PhantomReference<Object> phantomReference = new PhantomReference<>(new Byte[1024 * 1024], referenceQueue);
...
//        null
//        null
//        发生gc了，通过队列去拿到一个引用；虚引用gc后先放到队列里面，然后再被gc掉 
//        java.lang.ref.PhantomReference@1f89ab83
//        null
    }

```

- java.nio.ByteBuffer allocateDirect方法
